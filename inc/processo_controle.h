...
#define _processo_controle_
	...
	void setar_parametros_Kalman(float Q_acelerometro, float Q_magnetometro, float Q_bias, float Q_bias_mag, float R_acelerometro, float R_magnetometro);
	void setar_offset_acel(float offset[3]);
	void retornar_parametros_pid(float *Kp, float *Ki, float *Kd);
	void retornar_parametros_Kalman(float *Q_acelerometro, float *Q_magnetometro, float *Q_bias, float *Q_bias_mag, float *R_acelerometro, float *R_magnetometro);
