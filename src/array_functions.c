...
void inverter_vetor(uint8_t *buffer, uint8_t tamanho) //Inverte a posi��o dos elementos num vetor
{
    uint8_t counter = 0;
    uint8_t temp = 0;
...
uint8_t numToASCII(int16_t numero,uint8_t *buffer) //Converte um numero para caracteres ASCII na mesma ordem com que este est� escrito.
{
    uint16_t counter = 0;
...
void printfint(double numerof,uint8_t *destino)
{
    ...
    index+=tamanho;                                 //Posi��o ap�s a c�pia.
    destino[index] = CR;
    index++;
    destino[index] = LF;
